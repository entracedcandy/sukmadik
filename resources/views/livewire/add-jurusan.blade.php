...
>
    ...
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-left mb-3">
            <h4 class="fw-bold">Setting Jurusan</h4>
            <button type="button" class="btn btn-primary" @click="tambahModal.show()">Tambah Jurusan</button>
        </div>
            ...
            <tbody>
                @foreach ($query as $item)
                    <tr class="text-center">
                        ...
                        <td>{{ optional($item->kampus)->nama_kampus }}</td>
                        <td><button wire:click="delete({{ $item->id_jurusan }})" class="btn btn-outline-danger btn-sm px-2">Hapus</button></td>
                        <td><button class="btn btn-outline-info btn-sm px-2" data-bs-toggle="modal" data-bs-target="#modalDetail" wire:click="showDetail({{ $item->id_jurusan }})">Detail</button></td>
                        <td><button type="button" class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalEditKampus" wire:click="editKampus({{ $item->id_jurusan }})">Edit</button></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalDetailLabel">Detail Jurusan</h5>
                ...
                <div class="modal-body">
                    @if($selectedDetail)
                        <dl class="row">
                            ...
                            <dd class="col-sm-9">{{ $selectedDetail->nama_jurusan }}</dd>

                            <dt class="col-sm-3">Kampus</dt>
                            <dd class="col-sm-9">{{ optional($selectedDetail->kampus)->nama_kampus }}</dd>

                            <dt class="col-sm-3">Jumlah Prodi</dt>
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="storeJurusan">
                        <div class="mb-3">
                            ...
                            <input type="text" wire:model.defer="InsertJurusan" id="nama_jurusan" class="form-control" placeholder="Masukkan nama jurusan">
                            @error('InsertJurusan') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>

                        <div class="mb-3">
                            <label for="id_kampus" class="form-label">Kampus</label>
                            <select wire:model.defer="InsertKampus" id="id_kampus" class="form-select">
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="updateKampus">
                        <div class="mb-3">
                            ...
                            <input type="text" wire:model.defer="InsertJurusan" id="nama_jurusan_edit" class="form-control">
                            @error('InsertJurusan') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
